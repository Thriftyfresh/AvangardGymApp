...
class WomenDailyReportScreen extends StatefulWidget {
  const WomenDailyReportScreen({super.key});

  @override
  State<WomenDailyReportScreen> createState() => _WomenDailyReportScreenState();
}

class _WomenDailyReportScreenState extends State<WomenDailyReportScreen> {
  ...
  Color _receptColor(String name) {
    final colors = [Colors.pink, Colors.purple, Colors.teal, Colors.indigo, Colors.amber, Colors.red, Colors.cyan, Colors.green, Colors.blue, Colors.deepOrange];
                  ...
                  children: [
                    Expanded(
                      child: OutlinedButton.icon(
                        ...
                        label: Text(_formatDate(_selectedDate), style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                ...
                child: Card(
                  child: Padding(
                    ...
                    child: Row(
                      ...
                      children: [
                        Column(children: [
                          Text('${dayMembers.length}', style: const TextStyle(fontSize: 36, fontWeight: FontWeight.bold, color: Colors.pink)),
                          const Text('Members Joined', style: TextStyle(color: Colors.grey)),
                        ]),
                        Column(children: [
                          Text('${grouped.keys.length}', style: const TextStyle(fontSize: 36, fontWeight: FontWeight.bold, color: Colors.purple)),
                ...
                Expanded(
                  child: Center(
                    child: Column(mainAxisSize: MainAxisSize.min, children: [
                      ...
                      Text('No members joined on ${_formatDate(_selectedDate)}', style: TextStyle(color: Colors.grey[500])),
                    ]),
                  ),
                )
              else
                      ...
                      return Card(
                        ...
                        child: ExpansionTile(
                          leading: CircleAvatar(
                            ...
                            child: Text(receptionist[0].toUpperCase(), style: TextStyle(color: color, fontWeight: FontWeight.bold)),
                          ),
                          title: Text(receptionist, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                          subtitle: Text('${members.length} member${members.length == 1 ? '' : 's'}'),
                          trailing: Container(
                            padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                            decoration: BoxDecoration(color: color.withOpacity(0.1), borderRadius: BorderRadius.circular(20)),
                            child: Text('${members.length}', style: TextStyle(color: color, fontWeight: FontWeight.bold, fontSize: 16)),
                          ),
