...
class MembersScreen extends StatefulWidget {
  final String role;
  const MembersScreen({super.key, this.role = 'admin'});
...
class _MembersScreenState extends State<MembersScreen> with SingleTickerProviderStateMixin {
  ...
  List<MemberModel> _filtered(List<MemberModel> members, String tab) {
    ...
    List<MemberModel> list = tab == 'All'
        ? members
        : members.where((m) => m.status == tab.toLowerCase()).toList();
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              context.read<MemberBloc>().add(DeleteMember(member.id));
  ...
  int _daysUntilBirthday(String birthday) {
    ...
    try {
      ...
      if (parts.length == 3) {
        ...
        DateTime bday;
        if (a > 100) {
          bday = DateTime(now.year, b, c);
        } else if (c > 100 && a > 12) {
          bday = DateTime(now.year, b, a);
        } else {
          bday = DateTime(now.year, a, b);
        }
        if (bday.isBefore(now.subtract(const Duration(days: 1)))) {
  ...
  List<MemberModel> _upcomingBirthdays(List<MemberModel> members) {
    final results = <MemberModel>[];
    for (final m in members) {
      if (m.birthday.isEmpty) continue;
      final days = _daysUntilBirthday(m.birthday);
      if (days <= 30) results.add(m);
    }
        ...
        return Card(
          ...
          child: ListTile(
            ...
            leading: CircleAvatar(
              backgroundColor: days == 0 ? Colors.pink.withOpacity(0.15) : Colors.orange.withOpacity(0.15),
              child: Icon(Icons.cake_rounded, color: days == 0 ? Colors.pink : Colors.orange),
            ),
            ...
            trailing: Container(
              ...
              decoration: BoxDecoration(
                color: days == 0 ? Colors.pink.withOpacity(0.1) : Colors.orange.withOpacity(0.1),
                borderRadius: BorderRadius.circular(20),
              ),
              child: Text(
                days == 0 ? 'Today! 🎉' : '$days day${days == 1 ? '' : 's'}',
                style: TextStyle(
                  color: days == 0 ? Colors.pink : Colors.orange[800],
              ...
              builder: (context, state) {
                ...
                return TabBarView(
                  ...
                  children: _tabs.map((tab) {
                    ...
                    final members = _filtered(state.members, tab);
                    return _buildYearGroupedList(members);
                  }).toList(),
                );
              },
            ),
